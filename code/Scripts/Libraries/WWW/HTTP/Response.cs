...
namespace HTTP
{
        public class Response
        {
                ...
                protected List<byte> unchunkedData = new List<byte>();
                ...
                public void ReadFromStream( Stream inputStream )
                {
                        ...
                        if( GetHeader("transfer-encoding") == "chunked" ) {
                        	while (true) {
								...
								length = int.Parse( hexLength, NumberStyles.AllowHexSpecifier );
								for( int i = 0; i < length; i++ ) {
									output.WriteByte( (byte)inputStream.ReadByte() );
								}
								
								// ERIA Begin - aob
                        		unchunkedData.AddRange( output.ToArray() );
								// ERIA End
								...
								byte c;
								do {
									c = (byte)inputStream.ReadByte();
								} while( c != Request.EOL[1] );
								// ERIA End
                        	}
							...
							if( GetHeader("content-encoding").Contains("gzip") ) {
                        	   	bytes = UnZip( unchunkedData.ToArray() );
							} else {
                        		bytes = unchunkedData.ToArray();
							}
							// ERIA End
                        } else {
                        	// Read Body
