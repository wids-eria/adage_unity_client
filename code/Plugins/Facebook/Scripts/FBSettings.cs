...
public class FBSettings : ScriptableObject
{
...
#if UNITY_EDITOR		
					if(instance.pluginLocation == "")
					{
						MonoScript current = MonoScript.FromScriptableObject(instance);
						string[] path = AssetDatabase.GetAssetPath(current).Split('/');
						for(int i = 1; i < path.Length - 3; i++)
							instance.pluginLocation += path[i] + "/";
					}

					//Builds an Asset folder outside of the ADAGE plugin for storing project specific information
					string properPath = Path.Combine(Application.dataPath, facebookSettingsPath);
					if (!Directory.Exists(properPath))
					{
						string[] path = facebookSettingsPath.Split('/');
						string currentPath = "";
						for(int i = 0; i < path.Length; i++)
						{
							string currentFolder = "/" + path[i];
							if(!Directory.Exists(Application.dataPath + currentPath + currentFolder))
							{
								string parentFolder = "Assets" + currentPath;
	...
	//Init values with ADAGE defaults to assure no compile errors or crashes - Greg
	[SerializeField]
	private string[] appIds = new[] { "665506350177594" };
	[SerializeField]
	private string[] appLabels = new[] { "ADAGEDEFAULT" };
	//End Modification
	...
	public static string PluginLocation
	{
		get
		{
			return instance.pluginLocation;
		}
	}
